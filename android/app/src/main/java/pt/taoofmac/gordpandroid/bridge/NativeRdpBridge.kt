...
object NativeRdpBridge : RdpInputCallbacks {
    ...
    fun healthStatus(): String {
        ...
        return "backend=${backend.name}, running=${running.get()}, mode=$lastMode, auth=$auth, addr=$address, tls=$fingerprint, clients=${backend.activeConnections()}, accepted=${backend.acceptedConnections()}, authFailures=${backend.authFailures()}, handshakeFailures=${backend.handshakeFailures()}, input=$input, inputEvents=${backend.inputEvents()}, rdpeiContacts=${backend.rdpeiContacts()}, frames=${frameCount.get()}, sentFrames=${backend.framesSent()}, bitmapBytes=${backend.bitmapBytes()}, queued=${backend.queuedFrames()}, dropped=${backend.droppedFrames()}, inputScale=${inputCoordinateScale.get()}"
    }
