...
object NativeRdpBridge : RdpInputCallbacks {
    ...
    fun healthStatus(): String {
        ...
        val nsCodecRawBytes = backend.nsCodecRawBytes()
        val nsCodecSavedBytes = backend.nsCodecSavedBytes()
        val jpegCodecRawBytes = backend.jpegCodecRawBytes()
        val jpegCodecSavedBytes = backend.jpegCodecSavedBytes()
        val pngCodecRawBytes = backend.pngCodecRawBytes()
        val pngCodecSavedBytes = backend.pngCodecSavedBytes()
        val rfxCodecRawBytes = backend.rfxCodecRawBytes()
        val rfxCodecSavedBytes = backend.rfxCodecSavedBytes()
        return "backend=${backend.name}, running=${running.get()}, mode=$lastMode, security=$securityMode, failedAuth={$failedAuthPolicy}, auth=$auth, addr=$address, tls=$fingerprint, clients=${backend.activeConnections()}, accepted=${backend.acceptedConnections()}, authFailures=${backend.authFailures()}, handshakeFailures=${backend.handshakeFailures()}, input=$input, inputEvents=${backend.inputEvents()}, rdpeiContacts=${backend.rdpeiContacts()}, dvcFragments=${backend.dvcFragments()}, frames=${frameCount.get()}, sentFrames=${backend.framesSent()}, graphics=${backend.graphicsPath()}, bitmapBytes=${backend.bitmapBytes()}, bitmapRleFrames=${backend.bitmapRleFrames()}, bitmapRleBytes=${backend.bitmapRleBytes()}, bitmapRleSavedBytes=${backend.bitmapRleSavedBytes()}, nsCodecFrames=${backend.nsCodecFrames()}, nsCodecBytes=${backend.nsCodecBytes()}, nsCodecRawBytes=$nsCodecRawBytes, nsCodecSavedBytes=$nsCodecSavedBytes, nsCodecSavedPercent=${savedPercent(nsCodecRawBytes, nsCodecSavedBytes)}, jpegCodecFrames=${backend.jpegCodecFrames()}, jpegCodecBytes=${backend.jpegCodecBytes()}, jpegCodecRawBytes=$jpegCodecRawBytes, jpegCodecSavedBytes=$jpegCodecSavedBytes, jpegCodecSavedPercent=${savedPercent(jpegCodecRawBytes, jpegCodecSavedBytes)}, pngCodecFrames=${backend.pngCodecFrames()}, pngCodecBytes=${backend.pngCodecBytes()}, pngCodecRawBytes=$pngCodecRawBytes, pngCodecSavedBytes=$pngCodecSavedBytes, pngCodecSavedPercent=${savedPercent(pngCodecRawBytes, pngCodecSavedBytes)}, rfxCodecFrames=${backend.rfxCodecFrames()}, rfxCodecBytes=${backend.rfxCodecBytes()}, rfxCodecRawBytes=$rfxCodecRawBytes, rfxCodecSavedBytes=$rfxCodecSavedBytes, rfxCodecSavedPercent=${savedPercent(rfxCodecRawBytes, rfxCodecSavedBytes)}, bitmapCodecStreamStops=${backend.bitmapCodecStreamStops()}, rdpgfxFrames=${backend.rdpgfxFrames()}, rdpgfxBytes=${backend.rdpgfxBytes()}, rdpgfxStreamStops=${backend.rdpgfxStreamStops()}, h264Status=${backend.h264Status()}, h264Frames=${backend.h264Frames()}, h264Bytes=${backend.h264Bytes()}, h264KeyFrames=${h264KeyFrames.get()}, h264CodecConfig=${h264CodecConfigFrames.get()}, h264EncoderErrors=${h264EncoderErrors.get()}, h264Queued=${backend.h264QueuedFrames()}, h264Dropped=${backend.h264DroppedFrames()}, h264Submitted=${backend.h264SubmittedFrames()}, queued=${backend.queuedFrames()}, dropped=${backend.droppedFrames()}, inputScale=${inputCoordinateScale.get()}"
    }

    private fun savedPercent(rawBytes: Long, savedBytes: Long): String {
        if (rawBytes <= 0L || savedBytes <= 0L) {
            return "0.0"
        }
        return String.format(java.util.Locale.US, "%.1f", savedBytes.toDouble() * 100.0 / rawBytes.toDouble())
    }
