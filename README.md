...
- Graphics default to RDPGFX Planar when the client supports it, with raw 24-bit bitmap updates retained as compatibility fallback. Bitmap RLE is available only as an experimental opt-in fallback (`GO_RDP_ANDROID_ENABLE_BITMAP_RLE=1`) with diagnostics/saved-byte evidence, not as a negotiated/default release path. NSCodec/JPEG/PNG SurfaceBits emitters are also experimental/opt-in and report write/raw/saved/percentage diagnostics when exercised; RemoteFX now has an internal encoder hook plus the same diagnostics when supplied, but no production encoder yet. None of these are release defaults. H.264/AVC over RDPGFX AVC420 is experimental and not a release compatibility claim until a client advertises support without force-mode probes; use `h264Status` in diagnostics to classify fallback reasons, and keep physical-device/Microsoft-client performance and compatibility evidence pending.
...
That matrix covers slow-path bitmap fallback, opt-in bitmap RLE fallback with saved-byte evidence, opt-in NSCodec/JPEG/PNG SurfaceBits probes with selected/write/raw/saved/percentage fields, RemoteFX capability/encoder-hook evidence, RDPGFX Planar, fixture-payload transport-hook cases for ClearCodec/Progressive/AVC444/AVC444v2, forced `/gfx:AVC420`, and forced `/gfx` H.264 smoke cases. It explicitly separates implemented/default paths from experimental emitters and deferred families: RemoteFX/RFX, AVC444/AVC444v2, ClearCodec, and progressive codecs now have runtime hooks/wrappers but still need production encoders before they can be compatibility claims. See [docs/GRAPHICS_CODECS.md](docs/GRAPHICS_CODECS.md) for the codec coverage and decision matrix.
