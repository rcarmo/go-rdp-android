...
func main() {
	...
	md := fmt.Sprintf("# FreeRDP compatibility probe\n\n"+
		...
		s.ExitCode, s.TCPSeen, s.X224Seen, s.MCSSeen, s.BitmapSeen, s.FreeRDPLogSize, s.ServerLogSize, s.ScreenshotPNG, s.ScreenshotXWD, bullet(s.ServerPhases), bullet(s.ErrorLines))
...
func readTrim(path string) string { v := strings.TrimSpace(read(path)); if v == "" { return "unknown" }; return v }
func exists(path string) bool { st, err := os.Stat(path); return err == nil && st.Size() > 0 }
func must(err error) { if err != nil { panic(err) } }
func appendLimited(v []string, s string, max int) []string { if strings.TrimSpace(s) == "" { return v }; v = append(v, s); if len(v) > max { return v[len(v)-max:] }; return v }
func bullet(lines []string) string { if len(lines) == 0 { return "- none\n" }; return "- " + strings.Join(lines, "\n- ") + "\n" }
