...
run_case bitmap-planar 'GO_RDP_ANDROID_DISABLE_RDPGFX=1 GO_RDP_ANDROID_DISABLE_H264=1 GO_RDP_ANDROID_ENABLE_BITMAP_PLANAR=1' '' '/sec:nla /bpp:32'
...
run_case rdpgfx-progressivev2-fixture 'GO_RDP_ANDROID_DISABLE_H264=1 GO_RDP_ANDROID_ENABLE_PROGRESSIVE_CODEC=1' "-test-pattern -progressivev2-file $OUT/codec-fixture.bin" '/sec:nla /gfx'
...
progressive_v2_fixture = load("rdpgfx-progressivev2-fixture")
if not progressive_v2_fixture.get("active_seen") or not progressive_v2_fixture.get("rdpgfx_seen"):
    failures.append("ProgressiveV2 fixture case did not produce active RDPGFX evidence")
...
- Progressive now has production-path cases (`rdpgfx-progressive-encoded`, `rdpgfx-progressivev2-encoded`) and fixture-hook cases (`rdpgfx-progressive-fixture`, `rdpgfx-progressivev2-fixture`) so matrix evidence distinguishes V1/V2 production selection from transport-hook output; write counters are recorded when the client/timing accepts the path, but active RDPGFX evidence is the hard gate.
- AVC444 and AVC444v2 now have production-path cases (`rdpgfx-avc444-encoded`, `rdpgfx-avc444v2-encoded`) plus fixture-hook cases. Production cases use the built-in bounded base/aux payload encoders; fixture cases remain transport-hook smoke tests only. Write counters are recorded when selected paths emit, but active RDPGFX evidence is the hard gate until client acceptance is stable.
...
cat >"$OUT/codec-coverage.json" <<'JSON'
{
  "runtime_emitters": [
    ...
    {"name":"RDPGFX Progressive / other progressive codecs", "status":"partial-production-opt-in", "matrix_cases":["rdpgfx-progressive-encoded","rdpgfx-progressivev2-encoded","rdpgfx-progressive-fixture","rdpgfx-progressivev2-fixture","rdpgfx-planar"], "toggle":"GO_RDP_ANDROID_ENABLE_PROGRESSIVE_CODEC=1", "client_proof":"missing-production-client-proof", "fixture_hook":true, "production_encoder":true, "release_default":false, "default_enabled":false, "default_emission":"bounded-progressive-payload-or-planar-fallback"},
