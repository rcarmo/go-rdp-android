...
RDPGFX Planar is the current compressed graphics path and is covered by a FreeRDP `/sec:nla /gfx` CI proof gate. The next top-priority graphics workstream is full-spectrum H.264/AVC encoding using Android `MediaCodec`, with a negotiated path layered ahead of RDPGFX only when client capability and transport support are proven. The Android layer now has an initial `capture/H264Encoder.kt` scaffold with encoder-surface input, bitrate/frame-rate/keyframe controls, sync-frame requests, copy-out encoded access units, and error callbacks; it is not yet wired into MediaProjection startup or RDP transport. The remaining work is resize/orientation recovery, gomobile/server encoded-frame plumbing, RDP capability negotiation, diagnostics, and explicit fallback to RDPGFX Planar or bitmap when unavailable. Track the scope in `/workspace/workitems/10-next/go-rdp-android-h264-full-spectrum-encoding.md`.
