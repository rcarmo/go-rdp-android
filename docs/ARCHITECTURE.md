...
RDPGFX Planar is the current compressed graphics path and is covered by a FreeRDP `/sec:nla /gfx` CI proof gate. The next top-priority graphics workstream is full-spectrum H.264/AVC encoding using Android `MediaCodec`, with a negotiated path layered ahead of RDPGFX only when client capability and transport support are proven. That work likely requires an encoder-surface capture path, bitrate/frame-rate/keyframe controls, resize/orientation recovery, new gomobile/server encoded-frame plumbing, and explicit fallback to RDPGFX Planar or bitmap when unavailable. Track the scope in `/workspace/workitems/10-next/go-rdp-android-h264-full-spectrum-encoding.md`.
