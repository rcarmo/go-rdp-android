...
func BitmapRLEFrames() int64
func BitmapRLEBytes() int64
func BitmapRLESavedBytes() int64
func RDPGFXFrames() int64
func RDPGFXBytes() int64
func H264Frames() int64
func H264Bytes() int64
func H264SubmittedFrames() int64
func H264DroppedFrames() int64
func H264QueuedFrames() int64
...
Frames are copied into a bounded `FrameQueue`. The queue drops old frames when full, keeping the newest frame available for RDP encoding, and exposes submitted/queued/dropped counters for health reporting. This is preferable for remote desktop UX because stale frames are less useful than the latest screen state. Frame ingress validates dimensions, stride, data length, and integer-overflow cases before accepting buffers; the bitmap tiler and desktop scaler repeat those geometry checks before reading or allocating derived frames. A parallel bounded encoded-frame queue now accepts copied-out H.264/AVC access units from Android (`SubmitH264Frame`) for the experimental transport path, with the same latest-frame/drop-oldest behavior and strict payload-size bounds. When RDPGFX is negotiated, the confirmed RDPGFX capability advertises AVC420 (8.1 `AVC420_ENABLED`, or 10.0+ without `AVC_DISABLED`), and H.264 is not disabled, the server consumes queued encoded access units and wraps them as `RFX_AVC420_BITMAP_STREAM` payloads (one full-surface region plus quant/quality metadata) and emits them as RDPGFX `WireToSurface_1` frames using the AVC420 codec ID, coalescing to the latest queued unit before each write. Android forwards codec configuration both from `MediaCodec` codec-config buffers and from `MediaFormat` `csd-*` entries on output-format changes. Server-side codec-config access units are normalized to Annex B when possible, cached, and prepended to the first keyframe so clients receive decoder configuration before dependent frames; length-prefixed NAL units are converted to Annex B before transport, and IDR NAL units are treated as keyframes even if the Android keyframe flag is absent. If no encoded unit is available at finalization, it keeps RDPGFX Planar/bitmap fallback available while an H.264 stream goroutine waits for future encoded units. The queues are reusable across Android service restarts: restart paths drain stale frames, closed queues do not hang drain calls, and mobile server startup now reports immediate listen failures before the UI treats the server as running. Listener lifecycle cleanup is bounded as well: `Server.Serve` clears its active address and stops its context-watcher goroutine on any exit path, not only on context cancellation. The bridge also exposes lightweight runtime health values (`Addr`, `TLSFingerprintSHA256`, active/accepted connection counts, auth/handshake failure counts, decoded input events, RDPEI contact totals, DVC fragment counts, sent frame batches, selected graphics path, bitmap payload byte totals, opt-in bitmap RLE frame/byte/saved-byte totals, RDPGFX frame/byte totals, H.264 encoded access-unit frame/byte totals, and H.264 submitted/queued/dropped access-unit queue totals) that the Android UI combines with local mode/auth/input/frame state.
...
→ Control
→ FontList / FontMap
→ RDPGFX Planar updates when negotiated, experimental H.264/AVC when negotiated/proven, otherwise slow-path bitmap updates with optional bitmap RLE diagnostic fallback
...
Graphics prefer RDPGFX over `drdynvc` when the client supports `Microsoft::Windows::RDS::Graphics`. The server initiates the RDPGFX dynamic channel after activation, confirms client capabilities, creates/maps a surface, and sends Planar-codec no-alpha RLE frame data. Classic slow-path bitmap updates remain the compatibility fallback and regression oracle; frames are split into 80x80 tiles to stay within safe packet/length envelopes, and a per-session tile cache skips unchanged tiles after the first frame. When `GO_RDP_ANDROID_ENABLE_BITMAP_RLE=1` is explicitly set, compressible 24-bpp bitmap fallback rectangles can be emitted as legacy bitmap RLE updates with expansion rejection, `bitmap_rle_*` traces, and Android/gomobile saved-byte diagnostics; this remains a diagnostic fallback, not negotiated/default release behavior. The server selects session desktop dimensions from client core settings (and Confirm Active bitmap caps when present) and scales captured frames to negotiated desktop size before either RDPGFX or bitmap encoding.
...
MediaProjection → VirtualDisplay → ImageReader RGBA_8888
  ...
  → optional bitmap RLE diagnostic fallback when explicitly enabled
```
...
RDPGFX Planar is the current compressed graphics path and is covered by a FreeRDP `/sec:nla /gfx` CI proof gate. The next top-priority graphics workstream is full-spectrum H.264/AVC encoding using Android `MediaCodec`, with a negotiated path layered ahead of RDPGFX only when client capability and transport support are proven. The Android layer has `capture/H264Encoder.kt` with encoder-surface input, bitrate/frame-rate/keyframe controls, sync-frame requests, bounded copy-out encoded access units, codec-config extraction from buffers and `MediaFormat` `csd-*`, and error callbacks. A direct-service experimental `h264_capture` path routes MediaProjection output into the bounded gomobile encoded-frame queue; the server normalizes Annex-B/length-prefixed access units, preserves codec config across coalescing, gates emission on RDPGFX AVC420 capability by default, exposes `h264Status`, and can force transport only for protocol experiments. The remaining work is real-client AVC420 proof without force mode, physical-device resize/orientation/lifecycle validation, and performance comparison against RDPGFX Planar and bitmap fallback. Track the scope in `/workspace/workitems/10-next/go-rdp-android-h264-full-spectrum-encoding.md`.
