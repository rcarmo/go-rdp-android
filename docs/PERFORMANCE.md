...
This is intentionally simple and measurable. The bitmap-update builder now pre-sizes classic bitmap update payloads, the default non-RLE fallback path writes each tile directly into its final bitmap-update payload while hashing the same bytes for dirty-tile suppression, and disabled trace call sites avoid per-tile variadic argument allocations. The 320x240 fallback benchmark dropped from roughly 73 to 13 allocations/op and from roughly 0.49 MB/op to 0.25 MB/op while preserving the same tile/update shape. Larger full-frame fallbacks also avoid the separate per-tile bitmap data allocation and disabled-trace allocations, reducing 1280x720 from roughly 5.91 MB/621 allocs to 2.95 MB/145 allocs and 1920x1080 from roughly 13.28 MB/1529 allocs to 6.63 MB/337 allocs. The primary optimization targets are unchanged-tile suppression, adaptive frame pacing, optional downscaling, compressed bitmap/RDPGFX-style updates, and H.264/AVC video transport.
