...
| 2026-05-27 | GitHub Actions `encoding-matrix` job | CI encoding matrix | bitmap, bitmap-rle, nscodec-opt-in, jpeg-opt-in, png-opt-in, rfx-encoded, rfx-fixture, rdpgfx-planar, rdpgfx-planar-stream, rdpgfx-uncompressed, deferred RDPGFX hooks, negotiated H.264, forced H.264 | CI matrix completed successfully with separate RemoteFX production (`rfx-encoded`) and fixture (`rfx-fixture`) evidence and uploaded full matrix artifacts | run `26543284313`, job `78189527222`, artifact `encoding-matrix-artifacts` (`summary.md`, per-case `summary.json`, `codec-coverage.json`) | CI runner FreeRDP may reject explicit `/gfx:AVC420`; matrix now requires forced H.264 evidence from `/gfx` fallback while keeping `/gfx:AVC420` as opportunistic smoke evidence. |
