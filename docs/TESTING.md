...
Requirements: `xfreerdp3`/`xfreerdp`, `Xvfb`, `xwd`, and `python3`. The matrix starts `cmd/mock-server` with a test pattern and runs five NLA cases: slow-path bitmap fallback, opt-in bitmap RLE fallback, RDPGFX Planar with H.264 disabled, forced H.264 with `/gfx:AVC420`, and forced H.264 with `/gfx`. The matrix summary includes `bitmap_rle_seen`, RLE byte counts, and saved-byte evidence for the opt-in RLE case; `codec-coverage.json` also distinguishes implemented paths, upstream `go-rdp` codec metadata, and Android emitters that are still missing. It writes per-case FreeRDP logs, mock-server logs, screenshots, JSON/Markdown summaries, a top-level `summary.md`, and a machine-readable `codec-coverage.json`; it exits non-zero if bitmap, RDPGFX Planar, or forced H.264 evidence is missing. Treat the H.264 cases as protocol smoke evidence only unless the client advertises AVC420 without `GO_RDP_ANDROID_FORCE_H264=1`. The summary also lists observed RDPGFX capability advertisements plus missing/deferred encoding families. Bitmap RLE is reported as scaffold-only because the 24-bpp COPY-order encoder has unit coverage but no runtime negotiation/emission yet; NSCodec, RemoteFX/RFX, AVC444/AVC444v2, ClearCodec, progressive codecs, and JPEG/PNG bitmap codecs remain unimplemented.
