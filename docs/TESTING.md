...
| 2026-05-27 | GitHub Actions `encoding-matrix` job | CI encoding matrix | bitmap, bitmap-rle, nscodec-opt-in, jpeg-opt-in, png-opt-in, rfx-encoded, rfx-fixture, rdpgfx-planar, rdpgfx-planar-stream, rdpgfx-uncompressed, deferred RDPGFX hooks, negotiated H.264, forced H.264 | CI matrix completed successfully with separate RemoteFX production (`rfx-encoded`) and fixture (`rfx-fixture`) cases; accompanying FreeRDP compatibility traces still show Confirm Active `bitmap_codecs=0` (no RemoteFX advertisement) | run `26543284313`, jobs `78189527222` + `78189527225`, artifacts `encoding-matrix-artifacts` and `freerdp-compat-probe` | CI runner FreeRDP may reject explicit `/gfx:AVC420`; matrix now requires forced H.264 evidence from `/gfx` fallback while keeping `/gfx:AVC420` as opportunistic smoke evidence. Remmina validation is still pending in an environment where Remmina is available. |
