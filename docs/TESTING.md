...
| 2026-05-18 | mock server/H.264 fixture | FreeRDP CI informational `h264-gfx` | nla/gfx fallback | server-side forced AVC420 emission trace captured; client AVC420 support not proven | `freerdp-compat-probe` artifact from CI `26022000155`; `h264_write_seen=true`, `h264_write_count=1`, `h264_write_bytes=7`, `h264_reason=forced-by-env`, `avc420_exit_code=24` | Non-blocking diagnostic only; runner FreeRDP rejects `/gfx:AVC420` and falls back to `/gfx`. |
