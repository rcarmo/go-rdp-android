...
| gomobile build | Build `mobile.aar`, verify API, build Go-backed APK + AAB, inspect native libraries/signatures, and upload standalone debug APK/AAB artifacts | `gomobile-build-artifacts`, `go-rdp-android-go-backed-debug-apk`, `go-rdp-android-go-backed-debug-aab`, `go-rdp-android-mobile-aar` |
...
- Go-backed APK and AAB builds against `mobile.aar`, with native library/content inspection, AAB signature report, and standalone debug APK/AAB artifact uploads.
...
Release-tag staging now expects production signing secrets in GitHub Actions (`RELEASE_KEYSTORE_BASE64`, `RELEASE_KEYSTORE_PASSWORD`, `RELEASE_KEY_ALIAS`, `RELEASE_KEY_PASSWORD`) and fails fast if they are absent. On `v*` tags the gomobile job builds Go-backed release APK/AAB variants, uploads `go-rdp-android-go-backed-release-apk` and `go-rdp-android-go-backed-release-aab`, and the staging job signs those release artifacts rather than debug variants.
...
The default output directory is `encoding-matrix-artifacts/`, which is ignored by Git and removed by `make clean`.

Requirements: `xfreerdp3`/`xfreerdp`, `Xvfb`, `xwd`, and `python3`. The matrix starts `cmd/mock-server` with a test pattern and runs a broad NLA graphics suite: slow-path bitmap fallback, opt-in bitmap RLE fallback, NSCodec/JPEG/PNG/RemoteFX capability probes, RDPGFX Planar, opt-in RDPGFX Planar streaming, opt-in RDPGFX uncompressed diagnostics, production-vs-fixture probes for non-default RDPGFX codecs, negotiated H.264 gating, forced H.264 with `/gfx:AVC420`, and forced H.264 with `/gfx`. It records SurfaceBits bitmap-codec write/raw/saved-byte fields, derived saved percentages, plus `bitmap_codec_stream_stop_count` for SurfaceBits bitmap-codec stream experiments and `rdpgfx_frame_write_count` plus `rdpgfx_frame_stream_stop_count` for RDPGFX cases. The `h264-negotiated-gfx` case leaves H.264 enabled but unforced and expects active RDPGFX with no H.264 writes unless a client genuinely advertises AVC420 support. Cases require active streaming and only require codec-specific selected/write evidence when the client advertises the corresponding Bitmap Codecs entry or when the path is a release/default gate, so lack of client capability is recorded without making the matrix flaky. The matrix summary also includes `bitmap_rle_seen`, RLE byte counts, and saved-byte evidence for the opt-in RLE case. `codec-coverage.json` distinguishes `runtime_emitters`, upstream `go-rdp` codec metadata, `missing_runtime_emitters`, release defaults, and non-default experimental emitters; the missing-runtime list is expected to be empty now that ClearCodec, Progressive/ProgressiveV2, AVC444/AVC444v2, and RemoteFX have at least partial/minimal or opt-in production encoders. Remaining gaps are represented as `client_proof`, `release_default=false`, and per-codec notes rather than as absent runtime emitters. It writes per-case FreeRDP logs, mock-server logs, screenshots, JSON/Markdown summaries, a top-level `summary.md`, and a machine-readable `codec-coverage.json`; it exits non-zero if bitmap, RDPGFX Planar, or forced H.264 evidence is missing. Treat the H.264 cases as protocol smoke evidence only unless the client advertises AVC420 without `GO_RDP_ANDROID_FORCE_H264=1`. For operator transport experiments, `cmd/mock-server` also accepts encoded fixture payloads via `-rfx-file`, `-clearcodec-file`, `-progressive-file`, `-progressivev2-file`, `-avc444-file`, and `-avc444v2-file`; these exercise transport hooks and are kept separate from production encoder cases. The local matrix includes encoded and fixture cases for RemoteFX, ClearCodec, Progressive, ProgressiveV2, AVC444, and AVC444v2 to preserve production-vs-fixture distinction; with FreeRDP 3.5.1 in this environment, Confirm Active advertises `bitmap_codecs=0`, so RemoteFX selection/write evidence remains absent and documented as capability absence.
...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-05-21 | mock server/test pattern | FreeRDP CI `/sec:rdp`, `/sec:tls`, `/sec:nla` | rdp/tls/nla | automated bitmap fallback active-streaming gates pass | `freerdp-compat-probe` artifact from CI `26216722742` | Mock/test-pattern evidence, not physical Android hardware. |
| 2026-05-21 | mock server/test pattern | FreeRDP CI `/sec:nla /gfx` | nla/gfx | automated RDPGFX active-streaming proof gate passes | `freerdp-compat-probe` artifact from CI `26216722742`; `rdpgfx_seen=true` | Mock/test-pattern evidence, not physical Android hardware; small geometry for proof stability. |
...
| 2026-05-20 | mock server/local FreeRDP 3.15.0 | Manual local encoding matrix | bitmap, bitmap-rle, rdpgfx-planar, forced AVC420, forced `/gfx` fallback | all five paths reached active state; bitmap produced slow-path updates, opt-in bitmap RLE produced compressed fallback evidence with positive saved bytes, RDPGFX Planar produced graphics-channel activity without H.264 writes, and forced H.264 emitted AVC420 payloads under `/gfx:AVC420` and `/gfx`; matrix lists remaining unimplemented codec families separately | local `scripts/encoding-matrix.sh /workspace/tmp/rle-summary-matrix`; bitmap `active_seen=true`, `bitmap_seen=true`; bitmap RLE `bitmap_rle_seen=true`, `bitmap_rle_bytes=342`, `bitmap_rle_saved_bytes=11968`; RDPGFX `active_seen=true`, `rdpgfx_seen=true`, `h264_reason=disabled-by-env`; forced H.264 `/gfx:AVC420` `h264_write_count=30`, `h264_write_bytes=690`; forced H.264 `/gfx` `h264_write_count=30`, `h264_write_bytes=690`, `h264_reason=forced-by-env` | Local FreeRDP 3.15.0 accepts `/gfx:AVC420`, unlike the GitHub runner package; bitmap RLE is opt-in only, and H.264 remains forced-mode protocol smoke evidence, not negotiated client proof. |
| 2026-05-27 | GitHub Actions `encoding-matrix` job | CI encoding matrix | bitmap, bitmap-rle, nscodec-opt-in, jpeg-opt-in, png-opt-in, rfx-encoded, rfx-fixture, rdpgfx-planar, rdpgfx-planar-stream, rdpgfx-uncompressed, deferred RDPGFX hooks, negotiated H.264, forced H.264 | CI matrix completed successfully with separate RemoteFX production (`rfx-encoded`) and fixture (`rfx-fixture`) cases; FreeRDP Confirm Active traces show `bitmap_codecs=0`, so NSCodec/JPEG stayed capability-gated and PNG stayed operator-override-only (all three remain opt-in diagnostic paths, not release defaults) | run `26543284313`, jobs `78189527222` + `78189527225`, artifacts `encoding-matrix-artifacts` and `freerdp-compat-probe` | CI runner FreeRDP may reject explicit `/gfx:AVC420`; matrix now requires forced H.264 evidence from `/gfx` fallback while keeping `/gfx:AVC420` as opportunistic smoke evidence. Remmina validation is still pending in an environment where Remmina is available. |
| 2026-05-27 | GitHub Actions `encoding-matrix` + FreeRDP compatibility | ClearCodec behavior check | `rdpgfx-clearcodec-fixture`, `rdpgfx-deferred-codecs`, `/sec:nla /gfx` caps | FreeRDP advertises RDPGFX capability sets compatible with ClearCodec gating and accepts ClearCodec fixture `WireToSurface_1` writes (`rdpgfx_clearcodec_write_count=1`); production ClearCodec path remains opt-in and still needs broader client proof before defaulting | run `26545046046`, artifacts `encoding-matrix-artifacts` + `freerdp-compat-probe` (`rdpgfx_cap version=0x00080004/0x00080105 supported=true`) | Microsoft Remote Desktop/Remmina ClearCodec advertisement+acceptance evidence still pending. |
| 2026-05-28 | local `make encoding-matrix` + Go validation | Final codec-matrix policy check | bitmap, bitmap-rle, NSCodec/JPEG/PNG opt-ins, RemoteFX production/fixture, RDPGFX Planar/stream/uncompressed, ClearCodec/Progressive production-vs-fixture probes, AVC444/AVC444v2 fixtures, negotiated/forced H.264 | local matrix completed after keeping ClearCodec/Progressive non-default write counters informational rather than hard-gated; all cases reached active state under local FreeRDP 3.5.1, PNG override recorded raw/saved/percent evidence, RFX stayed unselected because Bitmap Codecs advertisement was absent, and forced H.264 emitted one AVC420 write in both force cases | local `encoding-matrix-artifacts/summary.md`; commit `661e3c4`; validation also passed `git diff --check`, `GOTMPDIR="$PWD/.gotmp" go test ./...`, and `GOTMPDIR="$PWD/.gotmp" go vet ./...` | Mock/test-pattern evidence only. Non-default codec promotion remains blocked on stable negotiated client proof and physical-device performance. |
| 2026-05-28 | GitHub Actions full CI | Post-plan verification | Go checks, race/fuzz smoke, Android debug build, gomobile AAR, encoding matrix, FreeRDP compatibility | latest post-plan documentation commit passed the full default CI set, including the encoding matrix and blocking FreeRDP compatibility probe | run `26588988822`, commit `e3395d9` | Emulator smoke and release staging skipped as expected for a normal push. |
| 2026-05-31 | GitHub Actions full CI | Release-readiness/version and artifact verification | version/tag preflight, Go checks, race/fuzz smoke, Android debug build, gomobile AAR/APK/AAB, encoding matrix, FreeRDP compatibility | `0.1.2` identifiers are aligned, `v0.1.2` is available, Go-backed debug APK/AAB standalone artifacts are uploaded, and the default CI set passed | run `26709026065`, commit `acef7b9`, artifacts `go-rdp-android-go-backed-debug-apk`, `go-rdp-android-go-backed-debug-aab`, `go-rdp-android-mobile-aar`, `gomobile-build-artifacts`, `encoding-matrix-artifacts`, `freerdp-compat-probe` | Emulator smoke and release staging skipped as expected for a normal push; controlled `v0.1.2` tag path still requires signing secrets. |
