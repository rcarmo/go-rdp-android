...
| Graphics | `bitmap_tile`, `bitmap_tile_skip`, `frame_resize`, `rdpgfx_caps_advertise`, `rdpgfx_cap`, `rdpgfx_caps_confirm`, `rdpgfx_pdu` | Bitmap fallback tile emission, dirty-tile suppression, desktop-size scaling, and RDPGFX capability/frame-channel evidence. |
...
| Android bridge | Android logcat tags `GoRdpAndroid`, `GoRdpAndroidService`, `GoRdpAndroidCapture`, `GoRdpAndroidGo` | Bridge/service/capture health. The health string exposes backend, mode, auth, address/fingerprint, connection/auth/input/RDPEI/DVC/frame counters, selected graphics path, bitmap/RDPGFX byte counters, queue depth/drop count, and input scale. |
