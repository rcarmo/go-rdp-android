...
Current evidence commit: `b8fa6fe` (`Add H264 transport opt-out scaffold`)
Latest referenced CI run: `25990736996` (`main` CI, success)
...
Latest checked artifact from CI run `25990736996`:
...
| Graphics | RDPGFX compressed path implemented; H.264 encoder/API/diagnostics scaffold added | RDPGFX (`Microsoft::Windows::RDS::Graphics`) over `drdynvc` is enabled by default and sends Planar-codec no-alpha RLE frames through server-initiated dynamic channels. The CI `/sec:nla /gfx` proof gate shows active RDPGFX negotiation/streaming while the existing 24-bit BGR slow-path bitmap gates remain available as explicit compatibility fallback and benchmark oracle. The next graphics workstream is full-spectrum Android `MediaCodec` H.264/AVC encoding layered ahead of RDPGFX only after client/transport support is proven; initial Android encoder-surface scaffolding, an experimental direct-service `h264_capture` MediaProjection path, bounded gomobile encoded-frame submission, Go/gomobile/Android frame/byte counters, encoded access-unit submitted/queued/dropped diagnostics, Android keyframe/config/error counters, server-side H.264 access-unit validation bounds, and `GO_RDP_ANDROID_DISABLE_H264=1` opt-out scaffolding exist, with user-facing UI and RDP transport still pending. See `/workspace/workitems/10-next/go-rdp-android-h264-full-spectrum-encoding.md`. Android/physical-device performance evidence is still pending. |
