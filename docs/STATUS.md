...
Current evidence commit: `a637457`
Latest referenced CI run: `26708505017` (`main` CI; version bump and release-tag availability preflight passed, default-tag-only release jobs skipped on main)
...
| Go-backed APK/AAB | Passing in CI | Go-backed debug APK/AAB build and native-library inspection; latest artifacts include manifest/classes plus Go JNI libraries for `arm64-v8a`, `armeabi-v7a`, `x86`, and `x86_64`; both debug APK and debug AAB are uploaded as standalone artifacts. Release tags additionally build Go-backed release APK/AAB variants and the release staging job now consumes those release artifacts before production signing. |
...
| Encoding matrix (CI/local) | Passing | Dedicated `encoding-matrix` job runs `make encoding-matrix` on Ubuntu with FreeRDP/Xvfb tooling and uploads `encoding-matrix-artifacts`; latest referenced CI success: run `26708505017` for commit `a637457`, with artifact inspection confirming the matrix summary and codec coverage files are present. Local matrix execution still requires FreeRDP/Xvfb tooling and may be unavailable in minimal API containers. |
...
Latest checked artifact from CI run `26708505017`:
...
| Graphics | RDPGFX compressed path implemented; experimental codec families remain gated | RDPGFX (`Microsoft::Windows::RDS::Graphics`) over `drdynvc` is enabled by default and sends Planar-codec no-alpha RLE frames through server-initiated dynamic channels; subsequent-frame RDPGFX streaming is available behind `GO_RDP_ANDROID_ENABLE_RDPGFX_STREAM=1` while CI/release gates keep the conservative initial-frame proof path until broader client soak evidence is collected. The blocking CI `/sec:nla /gfx` proof gate shows active RDPGFX negotiation/streaming, while `/sec:rdp`, `/sec:tls`, and `/sec:nla` keep the slow-path bitmap fallback honest by disabling RDPGFX. The codec matrix now explicitly separates release defaults, production encoders, fixture hooks, and missing client proof: Planar remains the only default compressed path; raw bitmap remains the fallback/oracle; NSCodec/JPEG/PNG/RemoteFX/ClearCodec/Progressive/ProgressiveV2/AVC444/AVC444v2 stay opt-in, fixture, or diagnostic until negotiated client evidence and target-device performance justify promotion. SurfaceBits codec diagnostics now expose raw/saved/saved-percent counters consistently for NSCodec, JPEG, PNG, and RemoteFX; ProgressiveV2 has its own fixture matrix path; and local microbenchmarks cover Planar, RemoteFX, ClearCodec, uncompressed RDPGFX, NSCodec, JPEG, and PNG builders. See `docs/GRAPHICS_CODECS.md`, `docs/PERFORMANCE.md`, `/workspace/workitems/10-next/go-rdp-android-h264-full-spectrum-encoding.md`, and `/workspace/workitems/10-next/go-rdp-android-graphics-codec-coverage.md`. |
...
- Release preflight clean/synced/version/latest-CI checks pass, and strict release preflight now verifies that the target `vVERSION` tag is available for the current HEAD. Current identifiers are `VERSION=0.1.2`, Android `versionName=0.1.2`, `versionCode=3`, and no `v0.1.2` tag currently exists upstream. Release-tag staging now consumes Go-backed release APK/AAB artifacts instead of debug variants, but that path still needs a controlled `v*` dry-run after signing secrets are present. When `gh` is missing, `scripts/release-preflight.go` falls back to the GitHub REST API with `GITHUB_TOKEN`/`GH_TOKEN`/`GITHUB_PICLAW_BOT`. Signing secret presence still cannot be confirmed from this automation token (`RELEASE_KEYSTORE_BASE64`, `RELEASE_KEYSTORE_PASSWORD`, `RELEASE_KEY_ALIAS`, and `RELEASE_KEY_PASSWORD` are not visible through the available API token). Controlled `v*` release-candidate/dry-run tagging remains blocked until the repository owner confirms those secrets.
...
- `docs/GRAPHICS_CODECS.md` for implemented/missing codec coverage, capability evidence, and codec decision rules.
