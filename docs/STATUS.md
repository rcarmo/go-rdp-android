...
Current evidence commit: `4c7953b` (`Normalize service policy extras`)
Latest referenced CI run: `25960888119` (`main` CI, success)
...
Latest checked artifact from CI run `25960888119`:
...
| Graphics | Release blocker | 24-bit BGR slow-path bitmap tiles with dirty-tile suppression remain the compatibility fallback and benchmark oracle. Public APK release now requires a negotiated compressed graphics path, preferably RDPGFX (`Microsoft::Windows::RDS::Graphics`) over `drdynvc`; see `/workspace/workitems/10-next/go-rdp-android-rdpgfx-compression.md`. |
