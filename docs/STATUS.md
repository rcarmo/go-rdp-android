...
Current evidence commit: `d48885c` (`Add CI gosec scan and triage probe findings`)
Latest referenced CI run: `25623146171` (`main` CI, success)
...
Latest checked artifact from CI run `25623146171`:
...
- Security defaults are not production-safe yet: secure password hashing/managed credential storage hardening, TLS certificate persistence, auth policy, rate limiting, and threat model remain pending.
