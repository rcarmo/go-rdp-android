...
Current evidence commit: `4b74aae` (`Use non-sticky RDP foreground service restarts`)
Latest referenced CI run: `25651942028` (`main` CI, success)
...
Latest checked artifact from CI run `25651942028`:
...
| Android capture | Functional prototype | MediaProjection + ImageReader capture skeleton with test-pattern mode, pacing/backpressure, optional downscale. Long-running server starts now always run as a foreground service with a notification Stop action, explicit UI stop routing, projection-revocation shutdown, and non-sticky restart policy so process death requires an explicit user restart. The UI shows compact backend/running/mode/frame/input-scale health state. Physical-device validation pending. |
