...
Current evidence commit: `de3849c` (`Fix bridge health state audit findings`)
Latest referenced CI run: `25701416748` (`main` CI, success)
...
Latest checked artifact from CI run `25701416748`:
...
| Android capture | Functional prototype | MediaProjection + ImageReader capture skeleton with test-pattern mode, pacing/backpressure, optional downscale. Long-running server starts now always run as a foreground service with a notification Stop action, explicit UI stop routing, projection-revocation shutdown, and non-sticky restart policy so process death requires an explicit user restart. The UI shows compact backend/running/mode/auth/listen-address/TLS-fingerprint/client-count/input-enabled/frame/input-scale health state. Physical-device validation pending. |
