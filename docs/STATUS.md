...
Current evidence commit: `7e3691b` (`Expose active RDP client count in Android health`)
Latest referenced CI run: `25688805095` (`main` CI, success)
...
Latest checked artifact from CI run `25688805095`:
...
| Android capture | Functional prototype | MediaProjection + ImageReader capture skeleton with test-pattern mode, pacing/backpressure, optional downscale. Long-running server starts now always run as a foreground service with a notification Stop action, explicit UI stop routing, projection-revocation shutdown, and non-sticky restart policy so process death requires an explicit user restart. The UI shows compact backend/running/mode/listen-address/TLS-fingerprint/client-count/input-enabled/frame/input-scale health state. Physical-device validation pending. |
