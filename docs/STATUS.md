...
Latest referenced CI run: `26545046046` (`main` CI; encoding matrix and FreeRDP compatibility artifacts referenced below)
...
| Encoding matrix (CI/local) | Passing | Dedicated `encoding-matrix` job runs `make encoding-matrix` on Ubuntu with FreeRDP/Xvfb tooling and uploads `encoding-matrix-artifacts`; latest referenced CI success: run `26545046046` (ClearCodec fixture acceptance evidence included). Local 2026-05-28 matrix also completed after keeping non-default experimental codec write counters informational until client proof is stable. |
