...
- optional legacy/extra codecs (bitmap RLE, NSCodec, RemoteFX/RFX, AVC444/AVC444v2, ClearCodec, progressive, JPEG/PNG bitmap codecs) — bitmap RLE is now an experimental opt-in (`GO_RDP_ANDROID_ENABLE_BITMAP_RLE=1`) with COPY/color-order coverage, expansion rejection, local matrix saved-byte evidence, and Android/gomobile diagnostics, but it is still not negotiated or enabled by default; NSCodec/JPEG/PNG SurfaceBits emitters are also experimental/opt-in with Android/gomobile and matrix selected/write/raw/saved/percent diagnostics; RemoteFX now has a bounded SurfaceBits wrapper, internal encoder hook, runtime counters, Android/gomobile diagnostics, and matrix write/raw/saved/percent fields when an encoder is supplied, but no production encoder yet; shared `go-rdp` codec metadata now covers NSCodec, RemoteFX/RFX, RDPGFX ClearCodec/progressive/AVC444/AVC444v2, and JPEG, while remaining runtime emitters stay deferred until client evidence justifies them; tracked in `/workspace/workitems/10-next/go-rdp-android-graphics-codec-coverage.md`
