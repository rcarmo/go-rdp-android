...
If these are missing, the `release-files` job fails before publishing artifacts. As of 2026-05-17, `gh secret list --repo rcarmo/go-rdp-android` from the automation token still returned no visible repository secrets, so a controlled `v*` release-candidate/dry-run tag remains blocked until the repository owner confirms those four signing secrets are present and correct.
