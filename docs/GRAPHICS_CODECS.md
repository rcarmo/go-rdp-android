...
| RemoteFX / RFX | Decoder/capability metadata upstream; Android capability gate and bounded SurfaceBits payload wrapper | No | `go-rdp` has RFX package coverage and public GUID metadata; Android now has `GO_RDP_ANDROID_ENABLE_RFX_CODEC=1` selection scaffolding, `rfx_codec_selected` deferred-emission traces when advertised, a bounded `SetSurfaceBits` payload wrapper for encoded RFX messages, and an internal encoder hook that can emit RFX SurfaceBits when an encoder is supplied. Default runtime emission still needs an actual production RFX encoder and client evidence because RemoteFX is deprecated/disabled in many clients. |
| RDPGFX AVC444 / AVC444v2 | Codec IDs upstream; Android deferred probes plus encoder-hooked WireToSurface seams | No | Shared IDs exist; Android now has `GO_RDP_ANDROID_ENABLE_AVC444=1` and `GO_RDP_ANDROID_ENABLE_AVC444V2=1` selection traces plus generic RDPGFX encoded-frame hooks that can emit `WireToSurface_1(codecID=AVC444/AVC444v2)` when future encoders are supplied. Production transport remains deferred behind negotiated AVC420 client proof and Android H.264 capture stabilization. |
...
The matrix currently exercises implemented paths, records observed RDPGFX capability advertisements from server traces, annotates the AVC-related flag bits (`0x10` = `AVC420_ENABLED`, `0x20` = `AVC_DISABLED`), reports SurfaceBits codec write/raw/saved/percent fields when NSCodec/JPEG/PNG/RemoteFX experiments emit payloads, runs fixture-payload transport-hook cases for RemoteFX, ClearCodec, Progressive, AVC444, and AVC444v2, writes a machine-readable `codec-coverage.json`, and then lists the missing production encoder families above so reports are explicit about coverage boundaries.
