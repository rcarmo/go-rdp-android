...
| NSCodec | Raw-plane encoder/decoder/capability metadata upstream; no Android emitter | No | `go-rdp` now has NSCodec decode utilities, a raw AYCoCg-plane encoder, and public GUID metadata; Android server still needs negotiated emitter work before runtime use. |
