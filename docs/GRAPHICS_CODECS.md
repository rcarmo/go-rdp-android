...
| --- | --- | --- | --- |
| RDP 5/6 bitmap compression / bitmap RLE | Scaffold only | No | A conservative 24-bpp COPY-order encoder and compressed bitmap-update builder exist as test-covered building blocks, but they are not negotiated or used at runtime yet; keep them behind future capability evidence/toggles while RDPGFX Planar is green. |
