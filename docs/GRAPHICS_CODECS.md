...
| --- | --- | --- | --- |
| RDP 5/6 bitmap compression / bitmap RLE | Scaffold only | No | A conservative 24-bpp COPY-order encoder exists as a test-covered building block, but it is not negotiated or used at runtime yet; keep it behind future capability evidence/toggles while RDPGFX Planar is green. |
