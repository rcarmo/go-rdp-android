...
| NSCodec | Raw-plane encoder/decoder/capability metadata upstream; Android SurfaceBits builder scaffold | No | `go-rdp` now has public NSCodec encode/decode utilities and GUID metadata; Android has a tested `SetSurfaceBits` builder for NSCodec payloads, but negotiated runtime emission is still pending. |
