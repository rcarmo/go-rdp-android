...
| NSCodec | Raw-plane encoder/decoder/capability metadata upstream; Android capability-gated SurfaceBits builder scaffold | No | `go-rdp` now has public NSCodec encode/decode utilities and GUID metadata; Android parses advertised NSCodec IDs, has an opt-in `GO_RDP_ANDROID_ENABLE_NSCODEC=1` selection gate, has a tested `SetSurfaceBits` builder for NSCodec payloads, and now emits an initial opt-in NSCodec update with `nscodec_selected`/`nscodec_write` traces before falling back to default bitmap behavior when the gate is off. |
