...
| JPEG/PNG bitmap codecs | JPEG GUID upstream; Android JPEG SurfaceBits builder scaffold | No | `go-rdp` exposes JPEG bitmap-codec GUID metadata. Android now has an opt-in `GO_RDP_ANDROID_ENABLE_JPEG_CODEC=1` selection gate, tested JPEG `SetSurfaceBits` builder, opt-in initial emission path, and matrix/summarizer fields; real client proof remains pending client Bitmap Codecs evidence. PNG has no current negotiated RDP output path here. |
