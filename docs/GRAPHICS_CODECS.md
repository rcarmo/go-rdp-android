...
| NSCodec | Raw-plane encoder/decoder/capability metadata upstream; Android capability-gated SurfaceBits builder scaffold | No | `go-rdp` now has public NSCodec encode/decode utilities and GUID metadata; Android parses advertised NSCodec IDs, has an opt-in `GO_RDP_ANDROID_ENABLE_NSCODEC=1` selection gate, has a tested `SetSurfaceBits` builder for NSCodec payloads, emits an initial opt-in NSCodec update with `nscodec_selected`/`nscodec_write` traces and `graphics=nscodec`/`nsCodec*` diagnostics, and the FreeRDP summarizer/matrix now records NSCodec selected/write counts when experiments trigger it; default behavior remains unchanged when the gate is off. |
| RemoteFX / RFX | Decoder/capability metadata upstream; Android capability gate | No | `go-rdp` has RFX package coverage and public GUID metadata; Android now has `GO_RDP_ANDROID_ENABLE_RFX_CODEC=1` selection scaffolding and `rfx_codec_selected` deferred-emission traces when advertised. Runtime emission still needs an encoder and client evidence because RemoteFX is deprecated/disabled in many clients. |
| RDPGFX AVC444 / AVC444v2 | Codec IDs upstream; Android deferred probes | No | Shared IDs exist; Android now has `GO_RDP_ANDROID_ENABLE_AVC444=1` and `GO_RDP_ANDROID_ENABLE_AVC444V2=1` deferred-selection traces, but actual transport remains deferred behind negotiated AVC420 client proof and Android H.264 capture stabilization. |
| RDPGFX ClearCodec | Codec ID upstream; Android deferred probe | No | Shared ID exists and `GO_RDP_ANDROID_ENABLE_CLEARCODEC=1` can trace plausible negotiated support, but no encoder/emitter is implemented. |
| RDPGFX Progressive / other progressive codecs | Codec IDs upstream; Android deferred probe | No | Shared IDs exist and `GO_RDP_ANDROID_ENABLE_PROGRESSIVE_CODEC=1` can trace plausible negotiated support, but the progressive pipeline still needs encoder/emitter work. |
| JPEG/PNG bitmap codecs | JPEG GUID upstream; Android JPEG/PNG SurfaceBits builders | No | `go-rdp` exposes JPEG bitmap-codec GUID metadata. Android now has an opt-in `GO_RDP_ANDROID_ENABLE_JPEG_CODEC=1` selection gate, tested JPEG `SetSurfaceBits` builder, opt-in initial emission path, Android diagnostics (`graphics=jpeg-codec`, `jpegCodec*`), and JSON/Markdown matrix/summarizer fields; real client proof remains pending client Bitmap Codecs evidence. PNG now has a tested `SetSurfaceBits` payload builder plus an operator-only `GO_RDP_ANDROID_ENABLE_PNG_CODEC_ID=<1..255>` emission override and Android diagnostics (`graphics=png-codec`, `pngCodec*`) for client-specific codec-ID experiments, but no current negotiated automatic RDP output path. |
