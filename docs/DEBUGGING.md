...
SurfaceBits bitmap-codec streaming is experimental and guarded by `GO_RDP_ANDROID_ENABLE_BITMAP_CODEC_STREAM=1`; it only applies after an opt-in NSCodec/JPEG/PNG command is selected and keeps default fallback behavior unchanged. NSCodec runtime emission is experimental and guarded by `GO_RDP_ANDROID_ENABLE_NSCODEC=1`; it only activates when the client advertises an NSCodec entry in the Bitmap Codecs capability set. Server traces include `bitmap_codec_cap` entries from Confirm Active plus `nscodec_selected`/`nscodec_write` when the opt-in initial NSCodec SurfaceBits update is built and sent. Android health also exposes `graphics=nscodec`, `nsCodecFrames`, and `nsCodecBytes` once an opt-in NSCodec update is emitted. JPEG bitmap-codec emission is experimental and guarded by `GO_RDP_ANDROID_ENABLE_JPEG_CODEC=1`; it depends on a client-advertised JPEG Bitmap Codecs entry and emits `jpeg_codec_selected`/`jpeg_codec_write` traces plus `graphics=jpeg-codec`, `jpegCodecFrames`, and `jpegCodecBytes` diagnostics when used. `GO_RDP_ANDROID_JPEG_QUALITY=<1..100>` tunes the experimental encoder quality and defaults to 80 when unset or invalid. RemoteFX selection scaffolding is guarded by `GO_RDP_ANDROID_ENABLE_RFX_CODEC=1`; because no encoder is wired yet, advertised RemoteFX capabilities produce `rfx_codec_selected ... emission=deferred reason=encoder-missing` and then fall back safely. Deferred RDPGFX codec probes are similarly gated: `GO_RDP_ANDROID_ENABLE_CLEARCODEC=1`, `GO_RDP_ANDROID_ENABLE_PROGRESSIVE_CODEC=1`, `GO_RDP_ANDROID_ENABLE_AVC444=1`, and `GO_RDP_ANDROID_ENABLE_AVC444V2=1` produce `rdpgfx_*_selected` deferred traces when the negotiated RDPGFX capability version/flags can plausibly support them. PNG has a tested SurfaceBits builder and an operator-only override, `GO_RDP_ANDROID_ENABLE_PNG_CODEC_ID=<1..255>`, for client-specific codec-ID experiments. This is deliberately not negotiated automatically because there is no known standard Bitmap Codecs PNG GUID in this implementation; successful override emission traces `png_codec_selected`/`png_codec_write` and exposes `graphics=png-codec`, `pngCodecFrames`, and `pngCodecBytes` diagnostics. `GO_RDP_ANDROID_PNG_COMPRESSION_LEVEL=<0|-1|-2|-3>` tunes PNG compression (`0` default, `-1` none, `-2` best speed, `-3` best compression) for operator experiments and defaults to Go's PNG default when unset or invalid.
