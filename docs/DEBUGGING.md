...
RDPGFX Planar is the default compressed path. For diagnostic comparisons only, `GO_RDP_ANDROID_ENABLE_RDPGFX_UNCOMPRESSED=1` sends an initial RDPGFX uncompressed frame and traces `rdpgfx_uncompressed_selected`; keep this off for release defaults because it is a bandwidth-heavy baseline.
