...
For `/sec:rdp`, `/sec:tls`, and `/sec:nla`, the gate requires `active_seen=true`, `bitmap_seen=true`, and a non-timeout shutdown (`exit_code != 124`) after screenshot capture. For `nla-gfx`, it requires `active_seen=true`, `rdpgfx_seen=true`, screenshot capture, and non-timeout shutdown. Use the summaries to locate the last successful server trace phase and the per-attempt logs to distinguish real protocol regressions from Xvfb/client startup flakiness. If a job hits the workflow timeout while traces show active streaming/screenshot evidence, rerun the failed FreeRDP job once before treating it as a protocol regression; persistent timeout or repeated missing `active_seen`/`bitmap_seen`/`rdpgfx_seen` should be triaged as blocking. RDPGFX can be forced off with `GO_RDP_ANDROID_DISABLE_RDPGFX=1`; the H.264 bring-up path has a parallel `GO_RDP_ANDROID_DISABLE_H264=1` opt-out scaffold so encoded transport can be disabled independently while preserving RDPGFX Planar fallback. For protocol experiments only, `GO_RDP_ANDROID_FORCE_H264=1` can force AVC420 emission after RDPGFX negotiation even when the client advertises AVC disabled; do not use this as a compatibility signal. Enable traces and look for `rdpgfx_h264_status` to confirm whether H.264 was blocked by env opt-out, missing RDPGFX readiness, or client AVC420 capability flags before inspecting `rdpgfx_h264_write` frame traces; the Android health/debug line also exposes the last observed `h264Status` reason (`not-observed`, `rdpgfx-not-ready`, `disabled-by-env`, `client-avc420-not-advertised`, `ready`, or `forced-by-env`) so device tests can classify fallback behavior without pulling server logs first; `scripts/summarize-freerdp.go` exposes these as `h264_status_seen`, `h264_ready`, `h264_version`, `h264_flags`, `h264_write_seen`, `h264_write_count`, `h264_write_bytes`, `h264_reason`, and `avc420_exit_code` in probe summaries. For protocol-only client experiments, the mock server can feed reusable encoded access units with `-h264-file <annexb-or-length-prefixed.h264> -h264-fps <n>`; Annex B files are split into start-code-delimited access units for replay, the first unit is queued immediately so short client probes do not wait one frame interval, pair that with `/gfx` and keep `GO_RDP_ANDROID_DISABLE_H264` unset to exercise the AVC420 path without needing Android MediaProjection. CI also runs a non-blocking `h264-gfx` probe artifact for this path with `GO_RDP_ANDROID_FORCE_H264=1`. It uses a tiny SPS/PPS/IDR Annex B fixture, first tries `/gfx:AVC420`; if the runner FreeRDP build rejects that option, it records the early exit and falls back to `/gfx` to exercise server-side AVC420 emission traces even when the client advertises AVC disabled. This is informational until we have a valid compatibility matrix and should not replace the blocking bitmap/RDPGFX Planar gates.

## Experimental codec debugging

RDPGFX Planar is the default compressed path. `GO_RDP_ANDROID_ENABLE_RDPGFX_STREAM=1` enables experimental subsequent-frame RDPGFX streaming and traces/counts `rdpgfx_frame_stream_stop` on write failure (`rdpgfxStreamStops` in Android health); keep it off for release gates until client soak evidence is expanded.  For diagnostic comparisons only, `GO_RDP_ANDROID_ENABLE_RDPGFX_UNCOMPRESSED=1` sends an initial RDPGFX uncompressed frame, traces `rdpgfx_uncompressed_selected`, and reports `graphics=rdpgfx-uncompressed`; keep this off for release defaults because it is a bandwidth-heavy baseline.

SurfaceBits bitmap-codec streaming is experimental and guarded by `GO_RDP_ANDROID_ENABLE_BITMAP_CODEC_STREAM=1`; it applies after an opt-in NSCodec/JPEG/PNG/RemoteFX command is selected, keeps default fallback behavior unchanged, and exposes `bitmapCodecStreamStops` in Android health when streaming writes stop. NSCodec runtime emission is experimental and guarded by `GO_RDP_ANDROID_ENABLE_NSCODEC=1`; it only activates when the client advertises an NSCodec entry in the Bitmap Codecs capability set. Server traces include `bitmap_codec_cap` entries from Confirm Active plus `nscodec_selected`/`nscodec_write` when the opt-in initial NSCodec SurfaceBits update is built and sent. Android health also exposes `graphics=nscodec`, `nsCodecFrames`, `nsCodecBytes`, `nsCodecRawBytes`, `nsCodecSavedBytes`, and `nsCodecSavedPercent` once an opt-in NSCodec update is emitted. JPEG bitmap-codec emission is experimental and guarded by `GO_RDP_ANDROID_ENABLE_JPEG_CODEC=1`; it depends on a client-advertised JPEG Bitmap Codecs entry and emits `jpeg_codec_selected`/`jpeg_codec_write` traces plus `graphics=jpeg-codec`, `jpegCodecFrames`, `jpegCodecBytes`, `jpegCodecRawBytes`, `jpegCodecSavedBytes`, and `jpegCodecSavedPercent` diagnostics when used. `GO_RDP_ANDROID_JPEG_QUALITY=<1..100>` tunes the experimental encoder quality and defaults to 80 when unset or invalid. RemoteFX runtime emission is guarded by `GO_RDP_ANDROID_ENABLE_RFX_CODEC=1`; it depends on advertised RemoteFX or RemoteFX-Image Bitmap Codecs capability, uses the default production single-tile RFX encoder when available, falls back safely on encoder rejection, and shares the SurfaceBits stream-stop counter when streaming is enabled. Android/gomobile diagnostics expose `graphics=rfx-codec`, `rfxCodecFrames`, `rfxCodecBytes`, `rfxCodecRawBytes`, `rfxCodecSavedBytes`, and `rfxCodecSavedPercent`. For local transport-only experiments, `cmd/mock-server` can load operator-provided encoded payload fixtures with `-rfx-file`, `-clearcodec-file`, `-progressive-file`, `-progressivev2-file`, `-avc444-file`, and `-avc444v2-file`; these are separate from production encoders and may not be accepted by real clients. Deferred RDPGFX codec probes are similarly gated: `GO_RDP_ANDROID_ENABLE_CLEARCODEC=1`, `GO_RDP_ANDROID_ENABLE_PROGRESSIVE_CODEC=1`, `GO_RDP_ANDROID_ENABLE_AVC444=1`, and `GO_RDP_ANDROID_ENABLE_AVC444V2=1` produce `rdpgfx_*_selected` deferred traces when the negotiated RDPGFX capability version/flags can plausibly support them. PNG has a tested SurfaceBits builder and an operator-only override, `GO_RDP_ANDROID_ENABLE_PNG_CODEC_ID=<1..255>`, for client-specific codec-ID experiments. This is deliberately not negotiated automatically because there is no known standard Bitmap Codecs PNG GUID in this implementation; successful override emission traces `png_codec_selected`/`png_codec_write` and exposes `graphics=png-codec`, `pngCodecFrames`, `pngCodecBytes`, `pngCodecRawBytes`, `pngCodecSavedBytes`, and `pngCodecSavedPercent` diagnostics. `GO_RDP_ANDROID_PNG_COMPRESSION_LEVEL=<0|-1|-2|-3>` tunes PNG compression (`0` default, `-1` none, `-2` best speed, `-3` best compression) for operator experiments and defaults to Go's PNG default when unset or invalid.

## Legacy bitmap RLE scaffold debugging

The classic RDP 5/6 bitmap-compression path is currently a scaffold only. `internal/rdpserver/bitmap_rle.go` contains a conservative 24-bpp COPY/color-order encoder with round-trip unit coverage, and `buildCompressedBitmapRLEUpdate(...)` can build a compressed Bitmap Update payload with `BITMAP_COMPRESSION | NO_BITMAP_COMPRESSION_HDR`; the builder rejects payloads that would expand beyond the uncompressed tile. Runtime emission is guarded by `GO_RDP_ANDROID_ENABLE_BITMAP_RLE=1`; leave it off by default unless collecting explicit legacy bitmap-compression evidence. The local encoding matrix asserts `bitmap_rle_*` trace evidence for the opt-in case, and Android diagnostics include `bitmapRleFrames`, `bitmapRleBytes`, and `bitmapRleSavedBytes` when a running gomobile backend emits compressed bitmap rectangles. If work resumes on this path, keep the default bitmap fallback uncompressed until a client capability trace proves the path is needed, then add a guarded runtime toggle plus FreeRDP/Remmina smoke evidence before enabling it by default.
